...
def localizacion(balizas, real, ideal, centro, radio, mostrar=0):
  ...
  imagen = [] # matriz registro de los valores de measurementProb para cada i, j.
  incremento = 0.1
  bestProb = 9999999
  bestPos = [0, 0]

  for j in np.arange(-1 * radio, radio, incremento):
    imagen.append([])
    for i in np.arange(-1 * radio, radio, incremento):
      # Mover ideal y calcular la diferencia entre sus medidas y las de real.
      ideal.set(centro[0] + i, centro[1] + j, ideal.orientation)
      measurements = ideal.sense(balizas)
      prob = real.measurement_prob(measurements, balizas)
      imagen[-1].append(prob)
      if (prob <= bestProb):
        # Guardar registro de la posicion de ideal mas adecuada respecto a real.
        bestProb = prob
        bestPos = [centro[0] + i, centro[1] + j]

  # Mover ideal a la mejor posicion registrada.
  ideal.set(bestPos[0], bestPos[1], ideal.orientation)
...
for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    ...
    # Obtenemos medidas desde real e ideal.
    idealMs = ideal.sense(objetivos)
    realMs = real.sense(objetivos)
    
    # Si la diferencia entre las medidas de cualquier baliza son mayores a un umbral, se localiza.
    for i in range(len(objetivos)):
      if (abs(idealMs[i] - realMs[i]) >= errorUmbral):
        localizacion(objetivos, real, ideal, [ideal.x, ideal.y], 1)
        break
